...
func MakeSquareQueryIn3Day(uid int, limit int, sq services.SquareSetting) (string, error, int) {
	...
	resQuery = fmt.Sprintf("user_id != %s and created_at > %s and created_at < %s and %s order by rand() limit %s", strconv.Itoa(uid), startTime, endTime, rule, strconv.Itoa(limit))
...
func MakeSquareQueryBefore3Day(uid int, limit int, sq services.SquareSetting) (string, error, int) {
	...
	resQuery = fmt.Sprintf("user_id != %s and created_at  < %s and %s order by rand() limit %s", strconv.Itoa(uid), endTime, rule, strconv.Itoa(limit))
	return resQuery, nil, 0
}
